...
class Pggd_Meta_Box {
	...
	private static function get_error_closing( $state_changed ) {
		if ( $state_changed ) {
			/*
			 * 「中途半端に変わっている」とだけ伝えると、認証プラグインの利用者は
			 * まず「保護が外れたのでは」と考える。実際には漏れる方向へは転ばないので、
			 * 安心してよい範囲を先に示してから、確認 → 正常なら終わり → 相談、
			 * の順に並べる。
			 * 逆のパターン（資格情報は書けて索引が落ちた）では開き直しても
			 * 画面は普通に「保護されています」と出るため、
			 * 「正しければそのままでよい」という出口が要る。
			 */
			return __( '保護が外れることはありませんが、保護の状態が中途半端に変わっている可能性があります。編集画面を開き直して現在の状態をご確認ください。表示が正しければ、そのままで構いません。繰り返し失敗する場合は、サイトの管理会社にご相談ください。', 'pageguard' );
		}
	...
	private static function get_notice_messages() {
		return array(
			...
			'save_failed_partial'       => array(
				'type'          => 'error',
				'class'         => 'notice-error',
				// 相談先の案内は締めの一文に集約する。ここに書くと、
				// 再試行を促していないのに「繰り返し失敗する場合は」が先に出て
				// 論理が飛ぶうえ、重い行動が軽い行動より前に並ぶ。
				'text'          => __( 'データベースへの保存に失敗しました。', 'pageguard' ),
				'state_changed' => true,
			),
		);
	}
